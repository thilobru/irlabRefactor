...
### fifth run: BERTsentiment+OCR+ImageClustering+QueryPreprocessing
...
for topic in topicsDic:
    ...
    result_ids_pro, result_ids_con = search_BERT_OCR_Clustering(
        query, num_results)
    i = 1
    for each in result_ids_pro:
        resultList.append([topic, "PRO", each[0], i, each[1],
                          "Boromir_BERT_OCR_Clustering"])
        i += 1
    i = 1
    for each in result_ids_con:
        resultList.append([topic, "CON", each[0], i, each[1],
                          "Boromir_BERT_OCR_Clustering"])
        i += 1
